...
class gfbcSingle{
  ...
  ###
  public function getDropdown( $data, $fieldObj, $sFieldArray, $pFieldId) {
    ...
    foreach( $data as $key => $value ) {
      ...
      if(  array_search($fieldId, $sFieldArray)  !== 0){ /* Hide all level other then first level */
  ...
  public function preRenderProccess($form){
    ...
    foreach($fieldIds as $index => $sFieldArray){
      foreach( $form['fields'] as &$field )  {
        # Populate select on the run
        $fieldPosition = array_search( $field['id'], $sFieldArray );
        if( $fieldPosition !== false ){
          ...
          $field->choices = $this->getDropdown( $dropDownArray, $field, $sFieldArray, $this->pFieldId[$index] ); 
        }
      }
    }
