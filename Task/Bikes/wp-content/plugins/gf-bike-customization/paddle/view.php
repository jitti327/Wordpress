...
class Custom_paddle_Table extends WP_List_Table {
  ...
  function custom_record(){
    ...
    $vendor      = $wpdb->prefix . "paddle_vendor";
    $paddle      = $wpdb->prefix . "paddle";
    ...
    function process_bulk_action() {
      ...
      if( 'deleted'===$this->current_action() ) {
        foreach($_REQUEST['user'] as $ids){
          $paddleStatus    =  $wpdb->delete($paddle , ['vendor_id' => $ids] , array('%d'));
          $vendorStatus =  $wpdb->delete($vendor , ['id' => $ids] , array('%d'));
          if(($paddleStatus === false) || ($vendorStatus === false)){
    ...
    */
    function process_delete_action() {
      ...
      if($task === 'delete') {
        $paddleStatus    =  $wpdb->delete($paddle , ['vendor_id' => $id] , array('%d'));
...
$testListTable = new Custom_paddle_Table();
...
<div class="wrap">        
  <div id="icon-users" class="icon32"><br/></div>
    ...
    <a href="<?php echo admin_url('admin.php?page=manage-paddle','paddle'); ?>&amp;action=add" class="page-title-action">Add New</a>
