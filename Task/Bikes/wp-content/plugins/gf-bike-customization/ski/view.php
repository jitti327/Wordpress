...
class Custom_ski_Table extends WP_List_Table {
  ...
  function custom_record(){
    ...
    function process_bulk_action() {
      ...
      if( 'deleted'===$this->current_action() ) {
        foreach($_REQUEST['user'] as $ids){
          $skiStatus    =  $wpdb->delete($ski , ['vendor_id' => $ids] , array('%d'));
          $vendorStatus =  $wpdb->delete($vendor , ['id' => $ids] , array('%d'));
          if(($skiStatus === false) || ($vendorStatus === false)){
    ...
    */
    function process_delete_action() {
      ...
      if($task === 'delete') {
        $skiStatus    =  $wpdb->delete($ski , ['vendor_id' => $id] , array('%d'));
...
$testListTable = new Custom_ski_Table();
...
<div class="wrap">        
  <div id="icon-users" class="icon32"><br/></div>
    ...
    <a href="<?php echo admin_url('admin.php?page=manage-ski','ski'); ?>&amp;action=add" class="page-title-action">Add New</a>
