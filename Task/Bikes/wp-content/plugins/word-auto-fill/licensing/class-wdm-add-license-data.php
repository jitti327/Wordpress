...
if (!class_exists('Licensing\WdmAddLicenseData')) {
    class WdmAddLicenseData
    {
        ...
        public function displayLicensePage()
        {
            ...
            # update_option('edd_'.$this->pluginSlug.LICENSE_KEY, VALID);
            ...
            ?>
            <tr>
                <td class="product-name"><?php echo $this->pluginName;
            ?></td>
                <td class="license-key">
                    <?php
                    if ($status == VALID || $status == EXPIRED || $previousStatus == VALID || $previousStatus == EXPIRED) {
                        ?>
                        <input id="<?php echo 'edd_'.$this->pluginSlug.LICENSE_KEY?>" name="<?php echo 'edd_'.$this->pluginSlug.LICENSE_KEY?>" type="text" class="regular-text" value="<?php esc_attr_e($licenseKey);
                        ?>" readonly/>
                    <?php
                    } else {
                        ?>
            ...
            ?>
                ...
                <td class="license-status"><?php $this->displayLicenseStatus($status, $previousStatus);
            ?></td>
                <td class="wdm-actions">
                    ...
                    if ($status !== false && ($status == VALID || $status == EXPIRED || $previousStatus == VALID || $previousStatus == EXPIRED)) {
                        ...
                        ?>
                        <input type="submit" class="wdm-link" name="<?php echo 'edd_'.$this->pluginSlug.'_license_deactivate';
                        ?>" value="<?php _e('Deactivate', $this->pluginTextDomain);
                        ?>"/>
                        <?php
                        if ($status == EXPIRED) {
                            ?>
                            <input type="button" class="button" name="<?php echo 'edd_'.$this->pluginSlug.'_license_renew';
                            ?>" value="<?php _e('Renew', $this->pluginTextDomain);
                        ...
                        ?>
                        <input type="submit" class="button" name="<?php echo 'edd_'.$this->pluginSlug.'_license_activate';
                        ?>" value="<?php _e('Activate', $this->pluginTextDomain);
                        ?>"/>
                    <?php
                    }
            ?>
        ...
        public function displayLicenseStatus($status, $previousStatus)
        {
            if ($status !== false) {
                if ($status == VALID || $previousStatus == VALID) {
                    ?>
                    <span style="color:green;"><?php _e('Active', $this->pluginTextDomain);
                    ?></span>
                <?php
                } elseif ($status == EXPIRED || $previousStatus == EXPIRED) {
                    ?>
                    <span style="color:red;"><?php _e('Expired', $this->pluginTextDomain);
                    ?></span>
                <?php
                } else {
                    ?>
        ...
        public function getRemoteData($action, $licenseKey)
        {
            ...
            return wp_remote_post(add_query_arg($apiParams, $this->storeUrl), array(
