...
class Custom_State_Table extends WP_List_Table {
  ...
  function custom_record(){
    ...
    $T1        = $wpdb->prefix . "state";   // First Table name
    $T2        = $wpdb->prefix . "country"; // Second Table Name

    if(!empty($_REQUEST['s'])){
      $queryPart = "
      WHERE
        `T1`.`name` LIKE '%".$_REQUEST['s']."%'
      OR
        `T2`.`name` LIKE '%".$_REQUEST['s']."%'
      ";
    }
    if(!empty($orderby)){
    ...
    function process_bulk_action() {
      ...
      if( 'deleted'===$this->current_action() ) {
        foreach($_REQUEST['user'] as $ids){
          $deleteQuery =  DeleteAction( "state" , $ids);
          if($deleteQuery == 0){
